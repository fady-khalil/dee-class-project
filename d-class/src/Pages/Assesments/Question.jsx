    ...
    <form className="text-white mx-auto mb-32" onSubmit={handleSubmit}>
      ...
      <div className="flex items-center gap-x-6 mt-14">
        <button
          ...
          } border rounded-xl p-2.5 text-lg text-white`}
        >
          <CaretLeft />
        </button>
        <button
          className="bg-primary text-white px-10 py-4 rounded-md capitalize flex-1"
          type="submit"
          disabled={selectedAnswers.length === 0}
        >
          {isLastQuestion ? t("general.submit") : t("general.next")}
        </button>
      </div>
    </form>
  );
