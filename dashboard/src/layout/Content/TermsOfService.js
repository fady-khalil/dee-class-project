...
const TermsOfService = () => {
  ...
  const loadData = async () => {
    ...
    setError(null)
    try {
      const response = await api.get('/admin/content/terms-of-service')
      if (response?.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.put('/admin/content/terms-of-service', formData)
      if (response?.success) {
        setSuccess('Terms of service saved successfully!')
        setHasData(true)
        setIsEditing(false)
        setTimeout(() => setSuccess(null), 3000)
      } else {
        setError(response?.message || 'Failed to save terms of service')
      }
    } catch (err) {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete the terms of service?')) {
      return
    }
    ...
    try {
      const response = await api.put('/admin/content/terms-of-service', {
        content: '',
        content_ar: '',
      })
      ...
      if (response?.success) {
        ...
        setHasData(false)
        setSuccess('Terms of service deleted successfully!')
        setTimeout(() => setSuccess(null), 3000)
      }
    } catch (err) {
      setError('Failed to delete terms of service')
      console.error(err)
    } finally {
      setSaving(false)
    }
      ...
      <CCard>
        ...
        <CCardBody>
          ...
          {hasData ? (
            ...
            <CAlert color="info">
              No terms of service configured yet. Click "Add Terms of Service" to create one.
            </CAlert>
          )}
        </CCardBody>
      </CCard>
  ...
  return (
    <CCard>
      <CCardHeader>
        <strong>{hasData ? 'Edit Terms of Service' : 'Add Terms of Service'}</strong>
      </CCardHeader>
      <CCardBody>
        {error && <CAlert color="danger">{error}</CAlert>}
        {success && <CAlert color="success">{success}</CAlert>}
